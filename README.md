...
默认保留“空间感、移动幅度、透视”三条滑杆和自动巡游开关，参数为 `0.62 / 0.56 / 1.15`，自动 X/Y 为 `0.36 / 0.18`。明确要求无控制条时才使用 `--controls hidden`。底图的“无 HUD”不自动隐藏预览控件。

单图深度点云使用自适应采样、深度感知柔边点与断层清理，限制转角以避免展示未知背面。`--demo on` 开启可暂停的侧面→正面→侧面演示；拖拽或重置停止演示。默认保留深度、点大小、焦平面和重置。深度处理延迟至 16-bit 量化，边缘感知平滑与网格断层剔除减少轮廓拉伸；旧 8-bit 输入仍可用，已丢失精度不能恢复。

`--preset` 仅保留 `immersive`，删除原本无效的 `v18/legacy` 别名。无底图的“某画风点云/景深”请求先生成并检查底图，再派生；已有底图直接复用。Skill 详细流程按“画风 × 来源 × 表现形式”按需加载。
