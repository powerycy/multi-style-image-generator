...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--grid", type=int)
    result.add_argument("--strength", type=float, default=1.0, help="Legacy displacement strength.")
    result.add_argument("--depth-scale", type=float)
    result.add_argument("--motion", type=float, default=0.56)
    result.add_argument("--perspective", type=float, default=1.15)
    result.add_argument("--auto-x", type=float, default=0.36)
    result.add_argument("--auto-y", type=float, default=0.18)
    result.add_argument("--blur", choices=("depth", "none"), default="none")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    elif args.spatial_mode == "pointcloud":
        output = out_dir / f"{image.stem}-pointcloud.html"
    else:
        output = out_dir / f"{image.stem}-spatial-v18.html"
    raw_output = (
    ...
    if args.spatial_mode == "pointcloud":
        ...
        options = mesh_viewer.ViewerOptions(
            depth_scale=args.depth_scale if args.depth_scale is not None else 0.62,
            motion=args.motion,
