...
- **默认人物融合**：当用户提供人物照片并要求生成图片时，默认将人物自然融入目标场景和世界观。保留脸型、五官关系、发型/发际线轮廓、体态和可识别气质等身份锚点；默认重设计服装、鞋履、道具、姿势、动作、光照和材质，并安排与场景有关的叙事动作。只有用户明确要求保留原服装、原姿势、原道具或不要换造型时，才保留照片中的对应元素。禁止站立合影、旅游打卡姿势、现代装与幻想环境割裂或照片抠图贴背景。
- **严格身份保留模式**：用户说“用我的脸、像我本人、保留我的脸、参考这张人物照片”，或把人物照片作为身份参考时自动启用。身份相似度优先于场景细节、特效和服装精致度；人脸可见时直接执行，人脸不可见、严重模糊或遮挡时再索取清晰人脸照。不得泛化成游戏角色脸、网红脸、不同年龄感或另一个人，也不得在没有质检证据时承诺“已保留本人脸”。
- **人物提示词与质检**：直接出图 prompt 必须包含统一重绘、身份锚点、自然同向视线和世界观融合要求。出图后按 `references/portrait-panorama-qa.md`，在正常观看尺寸和实际交付场景中检查身份、双眼、表情、服装动作、光照和构图；不要为了检查而过度放大远中景人物。按“通过 / 可用但需说明 / 不通过”分级：正常观看时不可见或不影响使用的轻微问题不否决图片，只有明显破坏身份、眼神、融合或构图的关键缺陷才触发重绘。
...
- **人物 360° 默认布局**：用户没有明确要求人物特写时，采用环境主导的远中景构图；人物全身位于地平线附近、距观察点约 8–12 米、占画面高度约 6–10%，避开顶部/底部极点与左右拼接缝。人物默认不应比约 8 米更靠近观察点；不要生成大型人物、全身近景、海报式正面构图、浅景深特写或合影姿势。
- **人物全景质检门槛**：先检查候选图的身份、眼神、表情、世界观服装和动作、人物比例、左右连续性与空间关系；平面检查后，运行 `scripts/create_panorama_viewer.py <2:1-image-path> --embed-image` 创建仅用于 QA 的本地预览，并在 WebGL 中检查前、后、左、右及接缝附近多个朝向。结果分为“通过 / 可用但需说明 / 不通过”：“通过”和“可用但需说明”都可交付 PNG 与 HTML，后者在最终回复中简要说明限制；只有正常观看时可见的明显陌生脸或异常眼神、严重抠图割裂、人物显著过大、拼接断裂或空间不可用等关键缺陷才判为“不通过”。自动重绘最多一次；一次重绘后仍“可用但需说明”时交付最佳版本，不自动发起第二次重绘。
...
- 360° 全景图直接出图时：达到 `references/portrait-panorama-qa.md` 中的“通过”或“可用但需说明”即可附上最终图片和全景预览 HTML；后者简要说明限制，只有“不通过”候选不得作为最终交付。若用户要求动态增强，再基于可交付的全景图创建动态预览。无法落盘时说明当前只能展示静态候选图。
- 最终回复只有在身份质检确实通过时才可说“保留了本人脸”；否则使用“参考了人物身份”或“保留相似气质”等准确表述，并说明限制。
