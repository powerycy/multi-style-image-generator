...
- **穿着和道具约束**：只把用户明确指定或参考图中实际存在的服装、配饰、道具和姿势作为约束，逐条写进 prompt；不得自行添加未指定的特征。照片里的品牌标识或可读包装文字应改成无品牌泛化道具，避免水印、乱码和侵权式标识。
- **照片场景转译**：用户明确指定某张图作为场景参考时，保留其中可见的主体空间关系，但要把场景整体转译进目标风格，而不是换一个风格背景再贴人。
- **多风格批量**：同一人物生成多种风格时，每一张的 prompt 都要重复用户指定的身份、穿着、姿态、参考图角色和“统一重绘，不要抠图/绿幕感/照片脸贴背景”约束，不得自行增加风格数量或参考特征。
- **像素风降级**：星露谷、潜水员戴夫等像素风不能保留照片级五官。优先保留用户明确指定或参考图中实际存在的身份锚点，例如主服装色块、姿势、发型轮廓和表情符号，同时明确整张图必须使用统一像素网格。
