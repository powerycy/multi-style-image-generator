...
- **人物提示词与质检**：直接出图 prompt 必须包含统一重绘、身份锚点、自然同向视线和世界观融合要求。出图后按 `references/portrait-panorama-qa.md` 放大检查身份、双眼、表情、服装动作、光照和构图；任一关键项失败时先针对性重绘，不把候选图作为最终结果。
...
- **人物全景质检门槛**：先检查候选图的身份、眼神、表情、世界观服装和动作、人物比例、左右连续性与空间关系；平面检查通过后，运行 `scripts/create_panorama_viewer.py <2:1-image-path> --embed-image` 创建仅用于 QA 的本地预览，并在 WebGL 中检查前、后、左、右及接缝附近多个朝向。任一关键项失败时重绘并替换候选，不把候选 PNG 或 QA HTML 作为最终交付；全部质检通过后，才把对应 PNG 和 HTML 作为最终结果。
...
- 360° 全景图直接出图时：只有通过 `references/portrait-panorama-qa.md` 中适用的最终质检门槛后，才附上最终图片和全景预览 HTML；若用户要求动态增强，再基于通过质检的全景图创建动态预览。无法落盘时说明当前只能展示静态候选图。
